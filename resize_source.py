...
def drop(event):
    global zipf, height, file
    if event.data:
        if event.widget == listbox:
            files = listbox.tk.splitlist(event.data)
            if zipbox.get():
                zipf = zipfile.ZipFile(
                    os.path.dirname(files[0]) + '\\' + os.path.splitext(os.path.basename(files[0]))[0] + '-batch.zip',
                    'w')
            ...
            for f in files:
                ...
                if os.path.exists(f) and os.path.isfile(f) and os.path.splitext(f)[1] == ".png":
                    ...
                    im.copy().resize((int(width / 2), int(height / 2)),
                                     img_filters[filters.index(filtersvar.get())]).save(
                        file.location + "\\" + os.path.splitext(file.name)[0][
                                               0:len(os.path.splitext(file.name)[0]) - 3] + str(
                            int(height / 2)) + ".png")
                    im.copy().resize((int(width / 4), int(height / 4)),
                                     img_filters[filters.index(filtersvar.get())]).save(
                        file.location + "\\" + os.path.splitext(file.name)[0][
                                               0:len(os.path.splitext(file.name)[0]) - 3] + str(
                            int(height / 4)) + ".png")
                ...
                if zipbox.get():
                    for i in range(1, 4):
                        if i == 1:
                            path = file.location + "\\" + os.path.splitext(file.name)[0][
                                                          0:len(os.path.splitext(file.name)[0]) - 3] + ".png"
                        else:
                            if i == 3: i = 4
                            path = file.location + "\\" + os.path.splitext(file.name)[0][
                                                          0:len(os.path.splitext(file.name)[0]) - 3] + str(
            ...
            if zipbox.get():
                ...
                listbox.dirs.append(
                    file.location + "\\" + os.path.splitext(os.path.basename(files[0]))[0] + '-batch.zip')
                zipf.close()
